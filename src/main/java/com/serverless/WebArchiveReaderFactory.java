...
public class WebArchiveReaderFactory implements ArchiveFileConstants
{
  ...
  private static final WebArchiveReaderFactory factory = new WebArchiveReaderFactory();

  /**
   * Shutdown any public access to default constructor.
   */
  protected WebArchiveReaderFactory()
  {
    super();
  }
   ...
   * @throws IOException
   */
  public static ArchiveReader get(final String arcFileOrUrl) throws MalformedURLException, IOException
  {
    return WebArchiveReaderFactory.factory.getArchiveReader(arcFileOrUrl);
  }

  protected ArchiveReader getArchiveReader(final String arcFileOrUrl) throws MalformedURLException, IOException
  {
    return getArchiveReader(arcFileOrUrl, 0);
  }

  protected ArchiveReader getArchiveReader(final String arcFileOrUrl, final long offset) throws MalformedURLException, IOException
  {

    return UsableURI.hasScheme(arcFileOrUrl) && arcFileOrUrl.indexOf(":") > 1 ? get(new URL(arcFileOrUrl), offset) : get(new File(arcFileOrUrl),
        offset);
  ...
  {
    return WebArchiveReaderFactory.factory.getArchiveReader(f);
  }

  protected ArchiveReader getArchiveReader(final File f) throws IOException
  {
   ...
   * @throws IOException
   */
  public static ArchiveReader get(final File f, final long offset) throws IOException
  {
    return WebArchiveReaderFactory.factory.getArchiveReader(f, offset);
  }

  protected ArchiveReader getArchiveReader(final File f, final long offset) throws IOException
  {
    ...
    else if (WARCReaderFactory.isWARCSuffix(f.getName()) || WARCReaderFactory.isWARCSuffix("wat.gz"))
    {
   ...
   * @throws IOException
   */
  public static ArchiveReader get(final String s, final InputStream is, final boolean atFirstRecord) throws IOException
  {
    return WebArchiveReaderFactory.factory.getArchiveReader(s, is, atFirstRecord);
  }

  protected ArchiveReader getArchiveReader(final String id, final InputStream is, final boolean atFirstRecord) throws IOException
  {
    ...
    else if (ArchiveReaderFactory.isWARCSuffix(id) || id.endsWith("wat.gz") || id.endsWith("wet.gz"))
    {
   ...
   * @throws IOException
   */
  public static ArchiveReader get(final URL u, final long offset) throws IOException
  {
    return WebArchiveReaderFactory.factory.getArchiveReader(u, offset);
  }

  protected ArchiveReader getArchiveReader(final URL f, final long offset) throws IOException
  {
    // Get URL connection.
  ...
  /**
   * Get an ARCReader.
   * Pulls the ARC local into whereever the System Property <code>java.io.tmpdir</code> points. It
   * then hands back an ARCReader that
   ...
   * @throws IOException
   */
  public static ArchiveReader get(final URL u) throws IOException
  {
    return WebArchiveReaderFactory.factory.getArchiveReader(u);
  }

  protected ArchiveReader getArchiveReader(final URL u) throws IOException
  {
  ...
  protected ArchiveReader makeARCLocal(final URLConnection connection) throws IOException
  {
    File localFile = null;
    if (connection instanceof HttpURLConnection)
    {
  ...
  {
    return f.getName().toLowerCase().endsWith(DOT_COMPRESSED_FILE_EXTENSION);
  }
